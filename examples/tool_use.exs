...
|> Enum.each(fn event ->
  case event do
    ...
    %Types.ToolUseEvent{name: name, input: input} ->
      IO.puts(:stderr, "\n>> Tool call: #{name}")
      IO.puts(:stderr, "   Input: #{inspect(input)}")

    %Types.ToolResultEvent{name: name, output: output} ->
      preview = if is_binary(output), do: String.slice(output, 0, 200), else: inspect(output)
      IO.puts(:stderr, "<< Tool result: #{name}")
